...
for chunk in pd.read_json(file_path, lines=True, chunksize=chunk_size):
    ...
    if not filtered_chunk.empty:
        cols = ['code', 'product_name', 'brands_lc', 'nutriscore_score', 'nutriments']
        filtered_chunk = filtered_chunk.loc[:, filtered_chunk.columns.intersection(cols)]
        # Add the nutrients columns that are a dict in nutriments
        nutriments_cols = ['fat_100g', 'carbohydrates_100g', 'sugars_100g',
                           'fiber_100g', 'proteins_100g', 'salt_100g']
        for nutriment in nutriments_cols:
            filtered_chunk[nutriment] = filtered_chunk['nutriments'].apply(
                lambda x: x.get(nutriment) if isinstance(x, dict) else 0)
        filtered_chunk['sum_nutrients_100g'] = filtered_chunk[nutriments_cols].sum(axis=1)

        # Remove rows where sum_nutrients_100g is below 100
        filtered_chunk = filtered_chunk[filtered_chunk['sum_nutrients_100g'] >= 100]
    ...
    if chunk_count % 2 == 0:
        ...
        if filtered_parts:
            ...
            df_high_nutriscore['url'] = "https://world.openfoodfacts.org/product/" + df_high_nutriscore[
                'code'].fillna('').astype(str).str.zfill(13)
...
df_high_nutriscore['url'] = "https://world.openfoodfacts.org/product/" + df_high_nutriscore['code'].fillna(
    '').astype(str).str.zfill(13)
